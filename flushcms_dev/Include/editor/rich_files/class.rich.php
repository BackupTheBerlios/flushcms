...
//    $this->value = htmlspecialchars($value);
  ...
  function draw(){
    ...
    if($this->caption) echo $this->caption.":<br>";
    ...
    if(!$rich_flag){
      echo '<iFrame id="rich_fs_iframe" style="display:none;z-index:999"></iFrame>';
    }
...
<nobr>
  ...
  <img id="PageProterties_'.$name.'" onclick="active_rich = '.$name_id.'; show_dialog(\'PageProperties\')" alt="'.$lang->item('PageProperties').'" src="'.$class_path.'rich_files/images/page.gif" align="absMiddle" width="20" height="20">
      ...
      if(isset($cms_curr_user) && $cms_curr_user && $cms_curr_user->id){
        echo '<img onclick="active_rich = '.$name_id.'; show_dialog(\'CreateBlock\')" alt="Create Block" src="'.$class_path.'rich_files/images/block.gif" align="absMiddle" width="20" height="20">';
      }
...
<textarea name="'.$name_area.'" id="'.$name.'_area_id" rows="20" cols="60" style="';
    ...
    echo 'border: black thin; height='.$this->height.'; width='.$this->width.'">'.$this->value.'</textarea>
<!-- text -->
...
';
    ...
    if($is_msie){
      ...
      if(!$rich_flag){
        echo '<iframe id="rich_temp_iframe" style="width:1; height:1;">&nbsp;</iframe>';
      }
	...
	var '.$name.'_rich_css = Array();
      ...
      if($this->default_stylesheet){
        ...
        foreach($this->default_stylesheet as $k=>$val){
          echo $name.'_id.document.createStyleSheet("'.$val.'");'."\n";
          echo $name.'_rich_css['.$k.'] = "'.$val.'";'."\n";
        }
  ...
  '.$name.'_files_url = "'.$this->files_url.'";';
      ...
      if($this->snippets && $this->is_visible('snippets')){
        foreach($this->snippets as $k=>$val){
          ...
          echo '  '.$name.'_snippets['.$k.'] = Array(\''.$snippet_name.'\', \''.$snippet_code.'\');'."\n";
        }
