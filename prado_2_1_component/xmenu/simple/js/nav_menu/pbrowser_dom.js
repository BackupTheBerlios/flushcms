...
if ((l1 = cdd__activate_onclick))
	document.addEventListener("mousedown", l83, 0);
l84 = 0;
...
vq_ismac = 0;
if ((cts) && (cts.toLowerCase().indexOf("mac") > -1))
	vq_ismac = 1;
l39_loaded();
;
function l39_loaded()
...
function l97(id)
{
	...
	{
		this.l98 = document.getElementById(this.pelm.getAttribute("cddid") + "_menu");
		tco = l78(this.pelm);
		this.addb = 0;
		if (!this.l98.getAttribute("cddisroot"))
			this.addb = parseInt(this.l98.style.borderTopWidth);
...
function l11(l2, is_h, index, id)
{
	if (id)
	{
		if (!is_h)
		{
			...
			tav = 0;
			if (ttbs != "none")
				tav += parseInt(l41(l2, "item_border_width", id, l41(l2, "menu_items_border_width", index))) * 2;
		...
		else
		{
			...
			tav = 0;
			if (ttbs != "none")
				tav += parseInt(l41(l2, "menu_border_width", index)) * 2;
	...
	else
	{
		...
		else
		{
			...
			if ((l41(l2, "divider_border_style", index) != "none") && (tdw))
				tdw += (l41(l2, "divider_border_width", index) * 2);
			tdex = index + "_";
			if (index == "_main")
				tdex = "";
...
function generate_all_html(l2, l12, l13, index, is_root)
{
	...
	if (window.cdd__default_cursor == "hand")
		this.dc = "pointer";
	else
		this.dc = "default";
	this.l14 = l12;
	...
	if (is_root)
	{
		...
		sd += "<div l2id='" + index + "' cddish=" + (this.l17 + 0) + " " + is_root + this.l16 + " cddisitem=0 cddid='cdditem" + this.l14 + "' id='cdditem" + l12 + "_" + l13 + "menu' style='z-index:" + (l90++) + ";cursor:" + this.dc + ";" + get_menu_styles(l2, "menu", index, l9, l94) + get_menu_styles(l2, "menu", index, l93, null, 1) + l11(l2, this.l17, index) + get_padding(l2, index) + get_borders(l2, index) + "'>";
	}
	else
		sd += "<div l2id='" + index + "' cddish=" + (this.l17 + 0) + " " + is_root + this.l16 + " cddisitem=0 cddid='cdditem" + this.l14 + "' id='cdditem" + l12 + "_" + l13 + "menu' style='z-index:" + (l90++) + ";cursor:" + this.dc + ";visibility:hidden;left:0;top:0;position:absolute;" + get_menu_styles(l2, "menu", index, l9, l94) + get_menu_styles(l2, "menu", index, l93, null, 1) + l11(l2, this.l17, index) + get_padding(l2, index) + get_borders(l2, index) + "'>";
	staw = "";
	staw = "width='100%' ";
	...
	while (l2["item" + (this.id = l13 + this.i)])
	{
		...
		sd += "<div id='cdditem" + this.l3 + "_hl' cddisroll=1 cddisitem=1 cddicobj='" + this.l3 + "' " + this.l22 + "  cddhl=" + (!(!(l2["item" + this.id + "_0"] + l2["url" + this.id] + l2["click" + this.id])) + 0) + " style='position:absolute;cursor:" + this.l21 + ";visibility:hidden;" + l26(l2, this.id, index, 1, l9, l94) + l26(l2, this.id, index, 1, l93, null, 1) + this.udi + get_padding(l2, this.id, index, 1) + get_borders(l2, this.id, index, 1) + "'>";
		l33(l2, this.id, "rel", this.l3, 1, l12);
		// Item display
		if(l2["postback" + this.id])
		{
			sd += "<a  HREF=\""+l2["postback" + this.id]+"\" style=\"color:"+  l41(l2, "menu_items" + l9[1] + "", index)  +";border:0;text-decoration:"+l41(l2, "menu_items" + l9[6] + "", index)+"\" >"+l41(l2, "item_roll", this.id, l2["item" + this.id]) + "</a></div>";
			sd += "<div cddid='cdditem" + this.l14 + "' cddx='" + this.l6[0] + "' cddy='" + this.l6[1] + "' " + this.l22 + " l25=" + l12 + " cddisitem=1 " + this.l91 + " id='cdditem" + this.l3 + "' style='cursor:" + this.l21 + "; " + l26(l2, this.id, index, 0, l9, l94) + l26(l2, this.id, index, 0, l93, null, 1) + this.udi + get_padding(l2, this.id, index, 1) + get_borders(l2, this.id, index, 1) + "'>";
			l33(l2, this.id, "rel", this.l3, 0, l12);
			
			sd += "<a id='"+l2["nameid" + this.id]+"' HREF=\""+l2["postback" + this.id]+"\" style=\"color:"+  l41(l2, "menu_items" + l9[1] + "", index)  +";text-decoration:"+l41(l2, "menu_items" + l9[6] + "", index)+"\" >"+l2["item" + this.id] + "</a></div>";
		}
		else
		{
	...
	for (this.md in this.child_id)
		new generate_all_html(l2, l12, this.md + "_", this.md);
};
function get_menu_styles(l2, l27, id, t_params, t_names, is_effect)
{
	l31 = "";
	was_effect = 0;
	if ((is_effect) && (id.toString() == "_main"))
		return "";
	for (q = 0; q < t_params.length; q++)
	{
		if ((l5 = l41(l2, l27 + t_params[q], id)) != undefined)
		{
			if (is_effect)
			{
				was_effect = 1;
			...
			{
				if (q == 9)
					l31 += t_names[q] + ":url(" + get_l80_path(l5, l2.uid) + ");";
				else
					l31 += t_names[q] + ":" + l5 + ";";
			}
		}
...
function l26(l2, id, l7, l29, t_params, t_names, is_effect)
{
	...
	for (q = 0; q < t_params.length; q++)
	{
		...
		while (1)
		{
			if ((l5 = l41(l2, "item" + t_params[q] + l8, id, l41(l2, "menu_items" + t_params[q] + l8, l7))) != undefined)
			{
				if (is_effect)
				{
					if (!was_effect)
...
{
	if (l28)
		rval = l41(l2, "item_border_width", index, l41(l2, "menu_items_border_width", l7));
	else
		rval = l41(l2, "menu_border_width", index);
	if (!rval)
		rval = 0;
	rt = "";
	for (ra in l92)
		rt += "border-" + l92[ra] + "-width:" + rval + ";";
	return rt;
};
function get_divider_borders(l2, index, is_h)
{
	...
	if (is_h)
		hpart = "width";
	rval = 0;
	if (l41(l2, "divider_" + hpart, index))
		rval = l41(l2, "divider_border_width", index);
	if (!rval)
		rval = 0;
	rt = "";
	for (ra in l92)
...
{
	if (l28)
		rval = l41(l2, "item_padding", index, l41(l2, "menu_items_padding", l7));
	else
		rval = l41(l2, "menu_padding", index);
	if (rval)
	{
	...
	if (((iid = l41(l2, "icon_" + l27, id)) != undefined) && (iid > -1))
	{
		...
		if (l27 == "rel")
		{
			...
			if (l27 == "abs")
			{
				...
				sd += "<div id='cdditem" + l3 + "_pointer' cdd_br=" + (l2["point_icon_image_br" + iid] + 0) + " style='position:absolute;z-index:" + l90 + ";visibility:hidden;top:0;left:0;" + icwh + "'><div cddisitem=0 cddid='cdditem" + l3 + "' style='position:absolute;" + icxy + "'>" + icstatic + "</div></div>";
	}
};
...
function l40(l2, l12, hid, t_hor)
{
	...
	if (t_hor)
	{
		if ((l5 = l41(l2, "divider_width", hid)))
			sd += "<td height='100%'><table  border=0 cellspacing=0 cellpadding=0 height='100%' style='" + p2 + "'><tr><td><div style='width:" + l5 + ";'>" + p3 + "</div></td></tr></table></td>";
	}
	else
	{
		if ((l5 = l41(l2, "divider_height", hid)))
			sd += "<tr><td style=''><div style='" + p2 + "height:" + l5 + ";'>" + p3 + "</div></td></tr>";
	}
};
function l41(l2, l42, id, l43)
{
	if (l2[l42 + id] != undefined)
		return l2[l42 + id];
	else
		if (l2[l42] != undefined)
			return l2[l42];
...
function l46(l49, l47, l48)
{
	if (l49 != l47)
	{
		if ((this.l50 = document.getElementById(l49.getAttribute("cddid"))))
		...
		if ((!l49.getAttribute("cddisroot")) && (l49 != l48) && (l49.style.visibility == "visible"))
		{
			...
			if ((abobj = document.getElementById(l49.getAttribute("cddid") + "l35")))
...
function l53(l54, top)
{
	...
	{
		if ((l87) && (l87.getAttribute("l25") != l54.getAttribute("l25")))
			l46_hl(l44(l87));
		l46_hl(this.l49, top);
		if (top)
		{
			...
			if ((l56 = l54.getAttribute("cddstatus")))
			{
				...
				l64();
			if ((l88) && (l88.id != l87.getAttribute("cddid") + "_pointer"))
				l65();
		}
		if (parseInt(this.l55.getAttribute("cddhl")))
			this.l55.style.visibility = "visible";
		this.l49.setAttribute("menuhl", l54.id);
	}
	if (!this.l49.getAttribute("cddisroot"))
		new l53(document.getElementById(this.l49.getAttribute("cddid")));
};
function l46_hl(l49, l39_forward)
{
	if (l49.getAttribute("menuhl"))
	{
		if ((l39_forward) && (nobj = document.getElementById(l49.getAttribute("menuhl") + "_menu")))
...
function l82(e, click)
{
	...
	while (l59)
	{
		if (l59.getAttribute)
		{
			if ((niq = l59.getAttribute("cddisitem")))
			{
				...
				if ((l60 = document.getElementById(l59.getAttribute("cddid") + "_pointer")))
				{
					...
					if (parseInt(l49.getAttribute("cddish")))
		...
		if (l59 == document.getElementsByTagName("html")[0])
			break;
		l59 = l59.parentNode;
	}
...
function l66(l60, l49, cx, l61, l62, l63, s1, s2, l67, l68, l69, l70)
{
	...
	if (parseInt(l60.getAttribute("cdd_br")))
		l60.style[s2] = (l63 + l49["offset" + l70]) + "px";
	else
		l60.style[s2] = (l63 - l60["offset" + l70]) + "px";
};
function l71(click, l48, l72, call_menu)
{
	...
	if ((l48) && (l48.style.visibility == "hidden"))
	{
		...
		l86 = l48;
		l84 = 1;
		if ((l54 = document.getElementById(l72.id + "l35")))
...
function l78(l79, l80)
{
	...
	do
	{
		if ((!l80) && ((!l79.getAttribute("cddisitem")) && ((l79.style.position == "absolute") || (l79.style.position == "relative"))))
			break;
		rc[0] += l79.offsetLeft;
		rc[1] += l79.offsetTop;
	}
