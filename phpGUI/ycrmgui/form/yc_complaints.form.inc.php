...
function display_complaints_main_tab_form () 
{
	...
	include(PATH_FORM."yc_complaints.form.php");
	$wb->right_control = $maintab = $tab;
	include(PATH_FORM."yc_complaints_tab.form.php");
	include(PATH_FORM."yc_complaints_category_tab.form.php");
	...
	wb_set_text(wb_get_control($maintab,IDC_COMPLAINTS_LIST), array(
	   array($wb->vars["Lang"]["lang_id"], 	60),
	   array($wb->vars["Lang"]["lang_category"],	60),
	   array($wb->vars["Lang"]["lang_complainanter"],80),
	   array($wb->vars["Lang"]["lang_reply"],	40),
	   array($wb->vars["Lang"]["lang_handleman"],	80),
	   array($wb->vars["Lang"]["lang_handledate"],	140),
	...
	wb_set_text(wb_get_control($maintab,IDC_COMPLAINTS_CATEGORY_LIST), array(
...
//
	wb_set_handler($wb->right_control, "process_complaints");
	
}

function reset_complaints_view () 
{
	global $wb;
	// Empty listview
	wb_delete_items(wb_get_control($wb->right_control,IDC_COMPLAINTS_LIST), null);
	
	$keyword = $wb->keyword;
	$where_is = " where 1 ";
	if ($keyword) 
	{
		$where_is .=" AND (title LIKE '%{$keyword}%' OR complainanter LIKE '%{$keyword}%' ) ";
	}
	...
	while ($wb->db->next_record()) 
	{
		...
		$row[] = $wb->db->f("complainanter");
		$row[] = $wb->db->f("reply");
		$row[] = $wb->db->f("handleman");
		$row[] = $wb->db->f("handledate");
		$row[] = $wb->db->f("state");
		$row[] = $wb->db->f("userid");
	...
	wb_create_items(wb_get_control($wb->right_control,IDC_COMPLAINTS_LIST), $data);
...
function reset_complaints_category_view () 
{
	global $wb;
	// Empty listview
	wb_delete_items(wb_get_control($wb->right_control,IDC_COMPLAINTS_CATEGORY_LIST), null);
	...
	// Create listview items
	wb_create_items(wb_get_control($wb->right_control,IDC_COMPLAINTS_CATEGORY_LIST), $data);
...
function del_selected_complaints () 
{
	global $wb;
...
function del_selected_complaints_category () 
{
	global $wb;
	
	if ($wb->del_ids) 
	{
		$category_table_name = $wb->setting["Settings"]["complaints_category_table"];
		$where_is =" WHERE id IN ($wb->del_ids) ";
...
function process_complaints ($window, $id, $ctrl, $lparam1=0, $lparam2=0) 
{
	global $wb;
	...
	switch($id) 
	{
		...
		case IDC_COMPLAINTS_CATEGORY_LIST:
			if($lparam1 == WBC_DBLCLICK) 
			{
				$current_rows = wb_get_text($ctrl);
				$current_id = $current_rows[0][0];
