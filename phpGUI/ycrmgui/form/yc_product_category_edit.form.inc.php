...
/**
 *
 * yc_product_category_edit.form.inc.php
 *
 * @package    core
 * @author     John.meng <arzen1013@gmail.com>
 * @author     ÃÏÔ¶òû
 * @author     QQ:3440895
 * @version    CVS: $Id: yc_product_category_edit.form.inc.php,v 1.1 2006/12/24 07:50:17 arzen Exp $
 */
 

function product_category_ctrl_mapping () 
{
	$ctrl_map = array(
...
function create_product_category_edit_dlg () 
{
	global $wb;
	
	include(PATH_FORM."yc_product_category_edit.form.php");
	
	//-------- view detail -------
	if ($id=$wb->current_ids) 
	{
		get_product_category_by_id ($winmain,$id);
//		wb_message_box($wb->mainwin,$id);//implode(",",$all_ctrl)
	...
	wb_set_handler($winmain, "process_product_category_edit");
	wb_set_visible($winmain, true);
	
}

function get_product_category_by_id ($parent,$id) 
{
	global $wb;
	
	$ctrl_map = product_category_ctrl_mapping ();
...
function insert_update_product_category ($parent) 
{
	global $wb;
	...
	$wb->db->query($sql);
//	include(PATH_FORM."yc_product.form.inc.php");
	reset_product_category_view ();
	
}

function process_product_category_edit ($window, $id, $ctrl) 
{
	global $wb;
	...
	switch($id) 
	{
		...
		case IDC_UPDATE:
			...
			break;
		case IDC_SAVE:
			insert_update_product_category ($window);
