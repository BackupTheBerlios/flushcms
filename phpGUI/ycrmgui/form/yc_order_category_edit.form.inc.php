...
/**
 *
 * yc_order_category_edit.form.inc.php
 *
 * @package    core
 * @author     John.meng <arzen1013@gmail.com>
 * @author     ÃÏÔ¶òû
 * @author     QQ:3440895
 * @version    CVS: $Id: yc_order_category_edit.form.inc.php,v 1.1 2006/12/24 11:46:48 arzen Exp $
 */
 

function order_category_ctrl_mapping () 
{
	$ctrl_map = array(
...
function create_order_category_edit_dlg () 
{
	global $wb;
	
	include(PATH_FORM."yc_order_category_edit.form.php");
	
	//-------- view detail -------
	if ($id=$wb->current_ids) 
	{
		get_order_category_by_id ($winmain,$id);
//		wb_message_box($wb->mainwin,$id);//implode(",",$all_ctrl)
	...
	wb_set_handler($winmain, "process_order_category_edit");
	wb_set_visible($winmain, true);
	
}

function get_order_category_by_id ($parent,$id) 
{
	global $wb;
...
function process_order_category_edit ($window, $id, $ctrl) 
{
	global $wb;
	...
	switch($id) 
	{
		...
		case IDC_UPDATE:
			...
			break;
		case IDC_SAVE:
			insert_update_order_category ($window);
