 ...
 * @author     ÃÏÔ¶òû
 * @author     QQ:3440895
 * @version    CVS: $Id: yc_agreement.form.inc.php,v 1.2 2006/12/24 11:46:48 arzen Exp $
 */
...
function del_selected_agreement () 
{
	global $wb;
...
function del_selected_agreement_category () 
{
	global $wb;
	
	if ($wb->del_ids) 
	{
		$category_table_name = $wb->setting["Settings"]["agreement_category_table"];
		$where_is =" WHERE id IN ($wb->del_ids) ";
	...
	switch($id) 
	{
		...
		case IDC_AGREEMENT_LIST:
			if($lparam1 == WBC_DBLCLICK) 
			{
				...
				include_once PATH_FORM."yc_agreement_edit.form.inc.php";
				create_agreement_edit_dlg ();
			}
		...
		case IDC_AGREEMENT_CATEGORY_LIST:
			if($lparam1 == WBC_DBLCLICK) 
			{
				$current_rows = wb_get_text($ctrl);
				$current_id = $current_rows[0][0];
				...
				$wb->current_form_state=false;
				$wb->current_action='update';
				include_once PATH_FORM."yc_agreement_category_edit.form.inc.php";
				create_agreement_category_edit_dlg ();
			}
