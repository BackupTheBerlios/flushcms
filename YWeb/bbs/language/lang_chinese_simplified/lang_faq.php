...
$faq[] = array("怎样改变我的等级?", "基本上您不能够直接的改变任何的等级(等级出现您的用户名称下面及个人档案中),绝大多数的论坛使用等级表示发帖数并且区分用户,举例来说版主和管理员可能有特别的等级.请不要胡乱张帖文章来增加您的等级,您或许可以找版主,否则论坛管理员只会降低您的计数.");
...
$faq[] = array("什么是表情符号?", "表情符号是使用短代码的代表图象来表示一些情感,例如 :) 表示愉快, :( 表示不开心.可以在发帖时看到所有的笑脸符号的列表,请不要大量使用笑脸符号在一张帖子里,它们可能造成阅读不便致使管理员必须修改或删除帖子.");
...
$faq[] = array("什么是版主?", "版主是独立的(或是团队),他们可以编辑或删除帖子,并且可以在他们所管理的版面中锁定,解锁,转移,删除或者分割主题.版主会阻止会员发表错误,离题或有争议的内容.");
$faq[] = array("什么是团队?", "会员团队是一个论坛管理员分祖管理会员的方式.(这可能不同于其他的论坛),而且每个团队能被分配个别的权限.这使得管理员容易去建立几个类似版主功能的用户,或是限定团队成员进入限定版面等等.");
...
$faq[] = array("我怎样才能成为团队管理员?", "会员团队是由论坛的管理员建立的,同时他们也会分配一位团队管理员. 如果您有兴趣创建一个团队首先应该联系管理员,尝试给他们发一封邮件吧.");
