 ...
 *
 *   $Id: viewforum.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 *
 ***************************************************************************/
...
$forumicon = ( !empty($forum_row['forum_icon']) ) ? '<img src="./images/icons/' . $forum_row['forum_icon'] . '" border="0">' : '';
...
$template->assign_vars(array(
	...
	'L_NEW_COMMEND' => $lang['New_Commend'],
	'L_NO_NEW_COMMEND' => $lang['No_New_Commend'],
	'L_POSTED' => $lang['Posted'],
	'L_JOINED' => $lang['Joined'],
	'L_AUTHOR' => $lang['Author'],
	'L_FORUM_COMMEND' => $lang['Forum_commend'],
	'U_VIEW_FORUM_COMMEND' => append_sid("commend.$phpEx?" . POST_FORUM_URL ."=$forum_id"),
...
{ 
	...
	{ 
		...
		{ 
			$forum_sub[$row['forum_sub']][$row['forum_id']] = '<a href='.append_sid('viewforum.'.$phpEx.'?' . POST_FORUM_URL .'='.$row['forum_id']).' class="forumlink">'. str_replace("<","",$row['forum_name']) . ' </a><img src="images/icon_minipost_lock.gif" alt="'.$lang['Forum_locked'].'" border="0">'; 
		}
		else 
		{ 
			$forum_sub[$row['forum_sub']][$row['forum_id']]='<a href='.append_sid('viewforum.'.$phpEx.'?' . POST_FORUM_URL .'='.$row['forum_id']).' class="forumlink">'. str_replace("<","",$row['forum_name']) . ' </a>'; 
		} 
	} 
} 
$db->sql_freeresult($result); 
	...
	{ 
		...
		{ 
			...
			{ 
				...
				$last_post = '<span class="genmed">' . $last_post_time . '<br />'; 
				$last_post .= ( $forum_data[$j]['user_id'] == ANONYMOUS ) ? ( ($forum_data[$j]['post_username'] != '' ) ? $forum_data[$j]['post_username'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $forum_data[$j]['user_id']) . '">' . $forum_data[$j]['username'] . '</a>'; 
				$last_post .= '&nbsp;<a href="' . append_sid("viewtopic.$phpEx?"  . POST_POST_URL . '=' . $forum_data[$j]['forum_last_post_id']) . '#' . $forum_data[$j]['forum_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" border="0" /></a></span>'; 
			} 
			else 
			{ 
...
if( $total_topics )
{
	...
	for($i = 0; $i < $total_topics; $i++)
	{
		...
		if( $topic_type_table != $topic_rowset[$i]['topic_type'] && $topic_type_check != 0 )
		{
	
			if( $topic_rowset[$i]['topic_type'] == POST_ANNOUNCE )
			{
				$s_table_lang = $lang['Post_Announcement'];
			}
			else if( $topic_rowset[$i]['topic_type'] == POST_GLOBAL_ANNOUNCE )
			{
				$s_table_lang = $lang['Post_global_announcement'];
			}
			else if( $topic_rowset[$i]['topic_type'] == POST_STICKY )
			{
				$s_table_lang = $lang['Post_Sticky'];
			}
			else if( $topic_rowset[$i]['topic_type'] == POST_NORMAL )
			{
				$s_table_lang = $lang['Topics'];
			}
