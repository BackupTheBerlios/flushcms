 ...
 *
 *   $Id: modcp.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 ***************************************************************************/
...
$commend = ( isset($HTTP_POST_VARS['commend']) ) ? TRUE : FALSE;
...
if ( !empty($topic_id) )
{
	...
	$forum_path=forum_path($topic_row,$forum_id);
}
else if ( !empty($forum_id) )
{
	$sql = "SELECT forum_name, forum_topics, forum_sub 
		FROM " . FORUMS_TABLE . "
	...
	case 'delete':
		...
		if ( $confirm )
		{
			...
			while ( $row = $db->sql_fetchrow($result) )
			{
				...
				$user_updated [] = "SELECT ug.user_id, g.group_id as g_id, u.user_posts, u.user_allow_ag, g.group_count, g.group_count_max, ".$row['poster_id']." as u_id 
			...
			if ( sizeof($user_updated) )
			{
				for($i = 0; $i < sizeof($user_updated); $i++)
				{
					...
					while ($group_data = $db->sql_fetchrow($result))
					{
						...
						elseif ( $user_already_added && $user_remove OR $group_data['user_allow_ag'] == 0)
						{
...
if ( !$db->sql_query($sql) )
{
	...
	case 'normalise':
		...
		if ($mode == 'commend' && !$is_auth['auth_commend'])
        {
           $message = sprintf($lang['Sorry_auth_commend'], $is_auth['auth_commend_type']);
           message_die(GENERAL_MESSAGE, $message);
        }
		if ( empty($HTTP_POST_VARS['topic_id_list']) && empty($topic_id) )
		{
        ...
        {
            $message = $lang['Topics_Commended'] . '<br /><br />' . $message;
        }
	...
	default:
        ...
        {
    		$template->assign_block_vars('switch_auth_commend', array());
        }
		...
		$sql = "SELECT t.*, u.username, u.user_id, p.post_time
			...
			ORDER BY t.topic_type IN (3,2,1) DESC, p.post_time DESC
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			else
			{
				...
				else if ( $row['topic_type'] == POST_COMMEND )
				{
					$folder_img = $images['folder_commend'];
					$folder_alt = $lang['Topic_Commend'];
				}
				else 
				{
