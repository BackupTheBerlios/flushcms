 ...
 *
 *   $Id: admin_forums.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 ***************************************************************************/
...
{ 
	...
	{ 
		...
		{ 
			...
			$forum_sub[$row['forum_sub']][] = "add_select('".$row[cat_id].",".$row[forum_id].",".$row[sort_sub].",".$row[main_sub]."','".str_replace("'", "\'", $row[forum_name])."','$s',".($row[sort_sub]+1).");"; 
	...
	{ 
		...
		$sub_list .="add_select('".$cat_id.",0,0,0','".str_replace("'", "\'", $cat_data[$i][cat_title])."','$s',0);"; 
		...
		{ 
			...
			$sub_list .= "add_select('".$forum_data[$cat_id][$j][cat_id].",".$forum_data[$cat_id][$j][forum_id].",".$forum_data[$cat_id][$j][sort_sub].",".$forum_data[$cat_id][$j][forum_id]."','".str_replace("'", "\'", $forum_data[$cat_id][$j][forum_name])."','$s',".($forum_data[$cat_id][$j][sort_sub]+1).");"; 
	...
	switch($mode)
	{
		...
		case 'editforum':
			...
			if ($mode == 'editforum')
			{
				...
				{
					if( preg_match("/^.*?\gif|jpg|bmp|png$/", $file) )
					{
						$forumicon[] = basename($file);
					}
				}
				@closedir($dir);
				$select_icon = '<option value="">' . $lang['Forum_icon_none'] . '</option>';
				for($i = 0; $i < count($forumicon); $i++)
				{
					if ($forumicon[$i] !='no_icons.gif')
					{
						$selected = ( $row['forum_icon'] == $forumicon[$i] ) ? ' selected="selected"' : '';
						$select_icon .= '<option value="' . $forumicon[$i] . '"' . $selected . '>' . $forumicon[$i] . '</option>';
					}
				}
			...
			$forum_icon_img = ( !empty($row['forum_icon']) ) ? '<img src="../images/icons/' . $row['forum_icon'] . '" border="0" name="forumicon">' : '<img src="../images/icons/no_icons.gif" border="0" name="forumicon">';
		...
		case 'createforum':
			...
			if(empty($forum_set)) 
			{ 
				message_die(GENERAL_ERROR, "Set Error"); 
			} 
			else 
			{ 
			...
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, forum_desc, forum_order, forum_status, forum_icon, forum_sub, sort_sub, main_sub, prune_enable" . $field_sql . ") 
				VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " . $c . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', " .$forum_sub.",".$sort_sub.",".$main_sub.",". intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ")";
			if( !$result = $db->sql_query($sql) )
			{
			...
			{ 
				...
				{ 
					...
					$sql = "UPDATE " . FORUMS_TABLE . " 
							SET forum_name = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', forum_desc = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', forum_status = " . intval($HTTP_POST_VARS['forumstatus']) . ", forum_icon = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', prune_enable = " . intval($HTTP_POST_VARS['prune_enable']) . " 
							WHERE forum_id = " . intval($HTTP_POST_VARS[POST_FORUM_URL]); 
				}else 
				{ 
					$sql = "SELECT count(forum_sub) as id 
					FROM " . FORUMS_TABLE . " 
					WHERE forum_sub = " . intval($HTTP_POST_VARS[POST_FORUM_URL]); 
					if( !$result = $db->sql_query($sql) ) 
					{ 
						message_die(GENERAL_ERROR, "Set Error"); 
					} 
					...
					if( $get_sub_Id[id]>0 ) 
					{ 
						message_die(GENERAL_ERROR, "This forum have sub forum, couldn't set this forum to sub forum"); 
					}
					...
					$main_sub=intval($temp[3]); 
					$sql = "UPDATE " . FORUMS_TABLE . " 
							SET forum_name = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', cat_id = " . $c . ", forum_desc = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', forum_status = " . intval($HTTP_POST_VARS['forumstatus']) . ", forum_icon = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumicon']) . "', prune_enable = " . intval($HTTP_POST_VARS['prune_enable']) . " 
...
if( $total_categories = $db->sql_numrows($q_categories) )
{
	...
	for($i = 0; $i < $total_categories; $i++)
	{
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			$forumicon = ( !empty($forum_rows[$j]['forum_icon']) ) ? '<img src="../images/icons/' . $forum_rows[$j]['forum_icon'] . '" border="0">' : '';
