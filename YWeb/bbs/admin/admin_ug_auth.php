 ...
 *
 *   $Id: admin_ug_auth.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 *
 ***************************************************************************/
...
//
$forum_auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_delete', 'auth_sticky', 'auth_announce', 'auth_vote', 'auth_pollcreate', 'auth_sellpost', 'auth_hide4reply', 'auth_hide4posts', 'auth_hide4fortune', 'auth_ban', 'auth_greencard', 'auth_bluecard', 'auth_globalannounce');
...
if ( isset($HTTP_POST_VARS['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
{
	...
	if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'admin' && $user_level != ADMIN )
	{
		...
		if ( $userdata['user_id'] != $user_id )
		{
			...
			$sql = "UPDATE " . AUTH_ACCESS_TABLE . "
				SET auth_view = 0, auth_read = 0, auth_post = 0, auth_reply = 0, auth_edit = 0, auth_delete = 0, auth_sticky = 0, auth_announce = 0, auth_sellpost = 0, auth_hide4reply = 0, auth_hide4posts = 0, auth_hide4fortune = 0
				, auth_ban = 0, auth_greencard = 0, auth_bluecard = 0, auth_globalannounce = 0  WHERE group_id = $group_id"; 
			if ( !($result = $db->sql_query($sql)) )
			{
	...
	else
	{
		if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'user' && $user_level == ADMIN )
		{
			...
			{
				$sql = "UPDATE " . AUTH_ACCESS_TABLE . "
					SET auth_view = 0, auth_read = 0, auth_post = 0, auth_reply = 0, auth_edit = 0, auth_delete = 0, auth_sticky = 0, auth_announce = 0, auth_sellpost = 0, auth_hide4reply = 0, auth_hide4posts = 0, auth_hide4fortune = 0, auth_globalannounce = 0 
					WHERE group_id = $group_id";
				if ( !($result = $db->sql_query($sql)) )
				{
