 ...
 *
 *   $Id: groupcp.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 *
 ***************************************************************************/
...
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim, &$userdata, &$online_status_img, &$online_status)
{
    ...
    $aim_img = ( $row['user_aim'] ) ? '<a href="http://friend.qq.com/cgi-bin/friend/user_show_info?ln=' . $row['user_aim'] . '" target="_blank"><img src="' . $php_install_url . $images['icon_aim'] . '" alt="' . $lang['AIM'] . $row['user_aim'] . '" title="' . $lang['AIM'] . $row['user_aim'] . '" border="0" />' . $row['user_aim'] . '</a>' : '';
    $aim = ( $row['user_aim'] ) ? '<a href="http://friend.qq.com/cgi-bin/friend/user_show_info?ln=' . $row['user_aim'] . '" target="_blank">' . $lang['AIM'] . $row['user_aim'] . '</a>' : '';
...
else if ( $group_id )
{
		...
		if ( !empty($HTTP_POST_VARS['add']) || !empty($HTTP_POST_VARS['remove']) || isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) || isset($HTTP_POST_VARS['grant_ungrant']) )
		{
			...
			else 
			{
				if ( ( ( isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) ) && isset($HTTP_POST_VARS['pending_members']) ) || ( isset($HTTP_POST_VARS['remove']) && isset($HTTP_POST_VARS['members']) ) )
				{
					...
					if ( isset($HTTP_POST_VARS['approve']) && $board_config['email_enabled'] )
					{
	...
	//
	$sql = "SELECT u.username, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, ug.user_pending, ug.group_moderator, u.user_allow_viewonline, u.user_session_time 
		FROM " . USERS_TABLE . " u, " . USER_GROUP_TABLE . " ug
		WHERE ug.group_id = $group_id
			AND u.user_id = ug.user_id
			AND ug.user_pending = 0 
			AND ug.user_id <> " . $group_moderator['user_id'] . " 
		ORDER BY ug.group_moderator DESC, u.username LIMIT $start, ".$board_config['topics_per_page']; 
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Error getting user list for group', '', __LINE__, __FILE__, $sql);
	}
