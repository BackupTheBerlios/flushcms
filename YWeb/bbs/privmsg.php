 ...
 *
 *   $Id: privmsg.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 *
 ***************************************************************************/
...
else if ( $mode == 'read' )
{
    ...
    $aim_img = ( $privmsg['user_aim'] ) ? '<a href="http://friend.qq.com/cgi-bin/friend/user_show_info?ln=' . $privmsg['user_aim'] . '" target="_blank"><img src="' . $php_install_url . $images['icon_aim'] . '" alt="' . $lang['AIM'] . $privmsg['user_aim'] . '" title="' . $lang['AIM'] . $privmsg['user_aim'] . '" border="0" /></a>' : '';
    $aim = ( $privmsg['user_aim'] ) ? '<a href="http://friend.qq.com/cgi-bin/friend/user_show_info?ln=' . $privmsg['user_aim'] . '" target="_blank">' . $lang['AIM'] . $privmsg['user_aim'] . '</a>' : '';
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit && !$error )
	{
		...
		if ( $mode != 'edit' )
		{
			...
			if ( $board_config['email_enabled'] && $to_userdata['user_notify_pm'] && !empty($to_userdata['user_email']) && $to_userdata['user_active'] )
			{
