 ...
 *   email                : mods@xore.ca
 *
 *   $Id: cash.php,v 1.1 2006/11/20 08:46:28 arzen Exp $
 *
 ***************************************************************************/
		...
		if ( ($target->id() != $donater->id()) && isset($HTTP_POST_VARS['cash']) && is_array($HTTP_POST_VARS['cash']) )
		{
			...
			while ( $c_cur = &$cash->currency_next($cm_i,CURRENCY_ENABLED | CURRENCY_DONATE) )
			{
					 ...
					 is_numeric($HTTP_POST_VARS['cash'][$c_cur->id()]) )
				{
					...
					if ( $amount > 0 && $userdata[$c_cur->db()] > 0 )
					{
...
//
	case "modedit":
		...
		if ( ($userdata['user_level'] != ADMIN) && ($userdata['user_level'] != MOD) && ($userdata['user_level'] != SUPER_MOD))
		{
			if ( $ref == 'viewprofile' )
			{
		...
		$mask = false;
		if ( ($userdata['user_level'] == MOD) || ($userdata['user_level'] == SUPER_MOD) ) 
		{
