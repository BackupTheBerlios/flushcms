 ...
 *   email                : mods@xore.ca
 *
 *   $Id: classes_cash.php,v 1.1 2006/11/20 08:46:27 arzen Exp $
 *
 *
 ***************************************************************************/
    ...
    class cash_viewtopic
    {
        ...
        function post_vars(&$postdata,&$userdata,$forum_id)
        {
			...
			if ($postdata['user_id'] != -1) {
            	...
            	if ( $cash->currency_count() && (($userdata['user_level'] == ADMIN) || (($userdata['user_level'] == MOD || ($userdata['user_level'] == SUPER_MOD)) && $cash->currency_count(CURRENCY_ENABLED | CURRENCY_MODEDIT | CURRENCY_VIEWTOPIC, $forum_id))) )
            	{
